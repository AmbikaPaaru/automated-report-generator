...
def _chat_model() -> ChatAnthropic:
    return ChatAnthropic(model=settings.anthropic_model, api_key=settings.anthropic_api_key, timeout=60, max_retries=2)
...
def plan_analysis(state: ReportState) -> ReportState:
    logger.info("job %s: asking Claude to propose charts + insights", state["job_id"])
